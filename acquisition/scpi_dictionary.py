"""
SCPI command dictionary for oscilloscope and SMU control.

This module is the modernized home of the constants and helper
functions that previously lived in ``old/dictionary_SCPI.py``. The
contents are kept verbatim so any legacy code that imported
``dictionary_SCPI`` keeps working; the acquisition adapters import from
here instead of the ``old/`` package.

The constants are referenced from the original RTA/RTO/Keysight
oscilloscope SCPI manuals used by the lab.
"""

numCounts = "ACQ:AVA?"
numCountsKey = ":ACQuire:SEGMented:COUNT?"
